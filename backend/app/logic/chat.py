...
class ChatLogic:
        ...
        try:
            ...
            if request.detail:
                if request.detail.chat_id:
                    ...
                    # update select file and folder
                    result_chat_history = self.chat_crud.get_by_chat_id(
                        request.detail.chat_id, login_user_id)
                    if not result_chat_history:
                        return OutputResponse.error(code="E0002", msg="Chat history not found")
                    self.chat_crud.update(
                        request.detail.chat_id,
                    ...
                    chat_history_model = ChatHistory(
                        ...
                        selected_file=request.selected_files,
                        selected_folder=request.selected_folders,
                        created_at=current_date_time,
                        created_by=login_user_id)
    ...
    def get(self, chat_id: str, user_id: str):
        ...
        try:
            # Accessing data from the request
            list_chat_history_detail = self.chat_crud.get_by_chat_id(
                chat_id, user_id)
            ...
            # ]
            chat = list_chat_history_detail["chat"]
            details = list_chat_history_detail["details"]

            data = {
                "chat": {
                    "selected_file": chat.selected_file,
                    "selected_folder": chat.selected_folder,
                },
                "details": [
                    {
                        "chat_detail_id": item.id,
    ...
    def get_all(self, login_user_id: str, search: str, page: int, limit: int):
        ...
        try:
            ...
            list_chat_history, total = self.chat_crud.get_all(
                login_user_id, search, page, limit)
            chat_histories = [
                {
                    "chat_id": item.id,
