...
import { BsBuildingAdd } from "react-icons/bs"; // Changed icon for department
import { useForm } from "react-hook-form";
// import PopupUpdateDepartment from "@/src/components/PopupUpdateDepartment"; // New component for updating department
import PopupCreateDepart from "@/src/components/PopupCreateDepart";
...
import {
  ...
  createDepart,
  deleteDepart,
} from "@/src/utils/ApiService"; // Updated API imports
import { useRouter } from "next/navigation";

// Updated Inputs for Department
type Inputs = {
  name: string;
  parent_id?: string; // parent_id is optional for top-level departments
};
...
export default function Page() {
  ...
  const [postsPerPage, setPostsPerPage] = useState(10); // Keeping for pagination logic
  ...
  const [isModalAddDepartment, setIsModalAddDepartment] = useState(false); // Changed for department
  const [isCheckAll, setIsCheckAll] = useState(false);
  const [blur, setBlur] = useState("none");
  const [departmentId, setDepartmentId] = useState(""); // Changed for department ID
  const [isOpenUpdateDepartment, setIsOpenUpdateDepartment] = useState(false); // Changed for department
  const [isModalDeleteDepartmentSuccess, setIsModalDeleteDepartmentSuccess] =
    useState(false); // Changed for department
  ...
  // To search - Note: getAllDepart doesn't seem to support search directly from API given
  // You might need to filter client-side or update your API.
  useEffect(() => {
    if (user && user.role == 1) {
      loadListDepart(valueSearch.searchQuery || "");
    }
  }, [valueSearch, user, currentPage, postsPerPage]); // Added postsPerPage to dependency array

  // Load list department
  const loadListDepart = async (search: string) => {
    try {
      const response = await getAllDepart();
      if (response?.status == 200) {
        setListDepartmentFilter(response.data.data.departs);
      } else {
  ...
  // Close all Modal
  const closeModal = () => {
    setIsOpenUpdateDepartment(false);
    setIsModalDelete(false);
    setIsModalAddDepartment(false);
    setIsPopupMessageConnectDB(false);
    setIsModalDeleteDepartmentSuccess(false); // Added for success modal
  };
  ...
  const openUpdateDepartmentPopup = (
    e: React.MouseEvent<HTMLButtonElement>,
  ) => {
    let valueId = e.currentTarget.value;
    setDepartmentId(valueId);
    setIsOpenUpdateDepartment(true);
  };
  ...
  const getStatePopup = (childData: boolean) => {
    setIsOpenUpdateDepartment(childData);
    setIsModalAddDepartment(childData);
    // Reload departments after creation/update for immediate UI refresh
    if (!childData) {
      loadListDepart(valueSearch.searchQuery || "");
    }
  };
  ...
  const checkAllHandler = () => {
    ...
    if (isCheckAll) {
      ...
      const departmentIds = listDepartmentFilter.map((item: any) => {
        return item["id"];
      });

      setSelectedItems(departmentIds);
    }
    const updatedValue = [...listCheckAll];
  ...
  const debouncedInput = useCallback(
    ...
    }, 500), // Adjusted debounce time for better responsiveness
    [],
  );
  ...
  const getValueNumberItemPage = (e: any) => {
    ...
    setPaginationNumber(Math.ceil(listDepartmentFilter.length / numberItem));
    setCurrentPage(1);
  };

  // Logic to get current departments for display based on pagination
  const indexOfLastPost = currentPage * postsPerPage;
  const indexOfFirstPost = indexOfLastPost - postsPerPage;
  const currentDepartments = listDepartmentFilter.slice(
    indexOfFirstPost,
    indexOfLastPost,
  );
              ...
              <div className={`bg-[#EFEFEE] sm:mx-36 mx-3 opacity-${opacity}`}>
                <div className="flex flex-col w-full justify-center">
                  ...
                  <div className="w-full text-center">
                    <div className="mb-2.5 flex w-full justify-between">
                      ...
                      <div>
                        <button
                          ...
                          onClick={() => setIsModalAddDepartment(true)}
                          disabled={spinner}
                        >
                          <span className="flex justify-center items-center">
                            <Icon
                              className="w-[25px] h-[40px] text-white mr-2"
                              as={BsBuildingAdd} // Changed icon
                            />
                    ...
                    <div className="border-2 border-solid border-gray-300 rounded-lg sm:overflow-x-hidden overflow-x-auto overflow-y-auto scroll-container">
                      <div className="max-h-[500px] ">
                        <table className="table-list-files sm:w[100px] w-[150px]">
                          ...
                          <tbody>
                            {currentDepartments.map((department: any) => (
                              <tr
                                key={department["id"]}
                              ...
                              >
                                ...
                                <td>
                                  {/* Assuming parent_id exists and you can map it to a parent department name */}
                                  <p>
                                ...
                                <td className="text-right px-[10px] ">
                                  <button
                                    ...
                                    onClick={openUpdateDepartmentPopup}
                                    disabled={spinner}
                                  >
                                    <p
                    ...
                    <Modal
                      isOpen={isModalDelete && selectedItems.length > 0} // Only open if items are selected
                      onClose={closeModal}
                      classNames={{
                    ...
                    {selectedItems.length === 0 &&
                      isModalDelete && ( // Display this only if no items are selected
                        <Modal
                    ...
                    {isModalAddDepartment === true && (
                      <PopupCreateDepart statePopup={getStatePopup} />
                    )}
                    {/* popup update department */}
                    {/* {isOpenUpdateDepartment === true && (
                    <PopupUpdateDepartment
                      department_id={departmentId}
                      statePopup={getStatePopup}
                    />
                  )} */}
                    {/* Popup delete department success */}
                    <Modal
                      isOpen={isModalDeleteDepartmentSuccess}
                      onClose={closeModal}
                      classNames={{
                        closeButton: "flex justify-end items-center",
                        wrapper: "flex justify-center items-center",
                            ...
                            <ModalFooter>
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  loadListDepart(valueSearch.searchQuery || ""); // Reload department list
                                }}
                              >
                                OK
                              </Button>
